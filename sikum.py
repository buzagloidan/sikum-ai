...
from db_utils import (
    ...
    get_async_db_connection as db_get_async_connection,
    resilient_db_operation as db_resilient_operation,
    query_cache,
    get_quiz_with_caching as db_get_quiz_with_caching,
    get_user_quizzes_with_caching as db_get_user_quizzes_with_caching,
    optimize_db as db_optimize_db,
    vacuum_db as db_vacuum_db,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Record this start command in user activity
    with db_get_connection() as conn:
        ...
        ''', (user_id, json.dumps({"timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S')})))
        conn.commit()
...
def optimize_db():
    ...
    with db_get_connection() as conn:
        ...
        # Set synchronous mode for better performance with acceptable safety
        ...
        # Set mmap size for faster reads
        c.execute("PRAGMA mmap_size=30000000")
...
def atomic_transaction(func):
    ...
    @wraps(func)
    async def wrapper(*args, **kwargs):
        async with db_get_async_connection() as conn:
            try:
...
async def start_saved_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE, quiz_id: int) -> None:
    ...
    try:
        ...
        @db_resilient_operation(fallback=lambda: False)
        ...
        @db_resilient_operation(fallback=lambda: None)
...
async def share_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE, quiz_id: int) -> None:
    ...
    with db_get_connection() as conn:
        ...
        # Check if quiz exists AND belongs to this user
        c.execute('SELECT 1 FROM saved_quizzes WHERE quiz_id = ? AND user_id = ?', (quiz_id, user_id))
        if not c.fetchone():
            # Handle response based on whether it's a callback query or direct message
            if update.callback_query:
                await update.callback_query.edit_message_text("❌ מבחן זה לא נמצא או אינו שייך לך.")
            else:
                await update.message.reply_text("❌ מבחן זה לא נמצא או אינו שייך לך.")
            return
